...
/** ✅ Sanitiza tags para nunca salvar lixo/undefined no Firestore */
function sanitizeTags(tags: any): any[] {
  if (!Array.isArray(tags)) return [];
  return tags
    .filter((t) => t && typeof t === "object")
    .map((t) => ({
      id: String(
        t.id ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`
      ),
...
export async function createActivity(activity: Omit<Activity, "id">) {
  ...
  const docData: Record<string, any> = {
    ...
    // ✅ IMPORTANTÍSSIMO: persistir tags/priority já na criação
    priority: (activity as any).priority ?? "media",
    tags: sanitizeTags((activity as any).tags),
  ...
  if (
    ...
    !isNaN(Number(activity.completedAt))
  ) {
  ...
  try {
    ...
    if (isPermissionError) {
      const userMessage =
        "Erro de permissão. Verifique se:\n" +
    ...
    if (isBlockedError) {
      ...
      try {
        console.warn(
          "Network error detected, waiting for pending writes and retrying..."
        );
        ...
        console.debug(
          "Successfully created activity with addDoc after retry, id:",
          docRef.id
        );
        return docRef.id;
      } catch (retryError: any) {
        const userMessage =
          "Erro de conexão bloqueado. Por favor:\n" +
    ...
    if (
      ...
      try {
        console.warn(
          "setDoc failed with internal assertion, trying addDoc as fallback"
        );
    ...
    throw new Error(
      `Erro ao salvar atividade: ${error.message || "Erro desconhecido"}`
    );
  }
}
...
export async function updateActivity(activityId: string, updates: Partial<Activity>) {
  ...
  if (updates.description !== undefined)
    updatesToSave.topics = updates.description ? [updates.description] : [];
  if (updates.dueDate !== undefined)
    updatesToSave.date = new Date(updates.dueDate).toISOString();

  // ✅ Sanitiza tags quando vierem no update (evita undefined e garante persistência ok)
  if ((updates as any).tags !== undefined) {
    updatesToSave.tags = sanitizeTags((updates as any).tags);
  }
...
// Kanban operations
export function subscribeToKanbanColumns(
  userId: string,
  callback: (columns: KanbanColumn[]) => void
) {
  try {
    const q = query(
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          columns.push({ id: docSnap.id, ...docSnap.data() } as KanbanColumn);
        });
        callback(columns);
      },
      (error) => {
...
export function subscribeToKanbanItems(
  userId: string,
  callback: (items: KanbanItem[]) => void
) {
  try {
    const q = query(
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          items.push({ id: docSnap.id, ...docSnap.data() } as KanbanItem);
        });
        callback(items);
      },
      (error) => {
...
export async function createKanbanColumn(column: Omit<KanbanColumn, "id">) {
  ...
  try {
    const newId = `${column.userId}-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 9)}`;
...
export async function createKanbanItem(item: Omit<KanbanItem, "id">) {
  ...
  // Use setDoc with generated ID to avoid subscription conflicts
  try {
    const newId = `${item.userId}-${item.activityId}-${Date.now()}-${Math.random()
      .toString(36)
      .substring(2, 9)}`;
...
export async function initializeKanbanColumns(userId: string) {
  ...
  try {
    ...
    existingColumns.forEach((docSnap) => {
    ...
    const promises = columnsToCreate.map(async (col, index) => {
      const newId = `${userId}-${Date.now()}-${Math.random()
        .toString(36)
        .substring(2, 9)}-${index}`;
      const docRef = doc(db, "kanban_columns", newId);
